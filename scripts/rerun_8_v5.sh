#!/bin/bash
# v5: slope-based plateau early-stop (single --plateau_window parameter).
# Same 8 experiments as v4, for direct comparison of episodes-to-convergence.
set -e
...
launch() {
    ...
    local SEG="$5"
    local MASK="$6"
    local OUTDIR="${RESULTS_BASE}/v5/experiments/reinforce_${NAME}_v5"
    local LOGFILE="logs/reinforce_${NAME}_v5.log"
    local MASK_ARG=""
    [ -n "$MASK" ] && [ -f "$MASK" ] && MASK_ARG="--mask $MASK"
    echo "[$(date)] Starting ${NAME}_v5 on GPU ${GPU} → ${LOGFILE}"
    # Defaults now: geo-mean reward, alpha=0.5, min_episodes=50, plateau_window=30.
    nohup $PYTHON -u generation/reinforce_search.py \
        ...
        > "$LOGFILE" 2>&1 &
    echo $! > "/tmp/reinforce_${NAME}_v5.pid"
}

echo "============================================"
echo "  REINFORCE v5: slope-based plateau stop"
echo "  $(date)"
echo "============================================"
...
echo ""
echo "Launched 8 v5 experiments on GPUs 0-7. Waiting..."
for NAME in catdog car_taxi sunflower_lavender chair_throne penguin_flamingo violin_guitar bgrich_teapot_globe bgrich_typewriter_laptop; do
    PID=$(cat /tmp/reinforce_${NAME}_v5.pid 2>/dev/null)
    [ -n "$PID" ] && { wait $PID 2>/dev/null || true; echo "[$(date)] ${NAME}_v5 finished"; }
done

echo ""
echo "============================================"
echo "  All 8 v5 DONE at $(date)"
echo "============================================"
