#!/bin/bash
# Rerun 8 representative experiments with the min_episodes counter fix.
# One wave on 8 GPUs. Uses same _v3 suffix (overwrites stale data).
set -e
...
launch() {
    ...
    local SEG="$5"
    local MASK="$6"
    local OUTDIR="${RESULTS_BASE}/v3/experiments/reinforce_${NAME}_v3"
    local LOGFILE="logs/reinforce_${NAME}_v3.log"
    local MASK_ARG=""
    [ -n "$MASK" ] && [ -f "$MASK" ] && MASK_ARG="--mask $MASK"
    echo "[$(date)] Starting ${NAME}_v3 on GPU ${GPU} → ${LOGFILE}"
    nohup $PYTHON -u generation/reinforce_search.py \
        ...
        > "$LOGFILE" 2>&1 &
    echo $! > "/tmp/reinforce_${NAME}_v3.pid"
}

echo "============================================"
echo "  REINFORCE 8-experiment validation (min_episodes fix)"
echo "  $(date)"
echo "============================================"
...
echo ""
echo "Launched 8 experiments on GPUs 0-7. Waiting for all..."
for NAME in catdog car_taxi sunflower_lavender chair_throne penguin_flamingo violin_guitar bgrich_teapot_globe bgrich_typewriter_laptop; do
    PID=$(cat /tmp/reinforce_${NAME}_v3.pid 2>/dev/null)
    [ -n "$PID" ] && { wait $PID 2>/dev/null || true; echo "[$(date)] ${NAME}_v3 finished"; }
done

echo ""
echo "============================================"
echo "  All 8 DONE at $(date)"
echo "============================================"
