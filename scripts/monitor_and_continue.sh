...
echo "=== Monitor: waiting for ${TARGET} images in ${OUTPUT_DIR} ==="
echo "Started at $(date)"

while true; do
    COUNT=$(ls "$OUTPUT_DIR" | wc -l)
    if [ "$COUNT" -ge "$TARGET" ]; then
        echo "Reached ${COUNT} images (target: ${TARGET}). Stopping generation..."
        # Kill the Python generation process and its children, then the bash wrapper
        kill -- -$(ps -o pgid= -p $PYTHON_PID | tr -d ' ') 2>/dev/null || true
        kill $PYTHON_PID 2>/dev/null || true
        kill $BASH_PID 2>/dev/null || true
        sleep 5
        echo "Generation stopped at $(date). Image count: $(ls "$OUTPUT_DIR" | wc -l)"
        break
    fi
    echo "$(date): ${COUNT}/${TARGET} images"
    sleep 30
done
...
$PYTHON metrics/segment_source.py \
    ...
    --dilate 10 \
    --threshold 0.5 \
    --output metrics/background_mask_room.npy \
    --save_vis
